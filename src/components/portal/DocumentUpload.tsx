...
import { Upload, File, Trash2, Loader2, CheckCircle2, XCircle, Plus, X, FileText, Edit, Calendar, User, Folder, LayoutGrid, Grid2X2, List, AlignJustify } from 'lucide-react'
...
type ViewMode = 'tiles' | 'large' | 'list' | 'compact'
...
export function DocumentUpload() {
  ...
  const [viewMode, setViewMode] = useState<ViewMode>(() => {
    if (typeof window !== 'undefined') {
      ...
      if (uid) {
        const saved = localStorage.getItem(`kb_viewMode_${uid}`)
        if (saved && ['tiles', 'large', 'list', 'compact'].includes(saved)) return saved as ViewMode
      }
    }
    return 'tiles'
  })
  ...
  useEffect(() => {
    if (user?.id) {
      localStorage.setItem(`kb_viewMode_${user.id}`, viewMode)
    }
  }, [viewMode, user?.id])
    ...
    <div className="h-full flex flex-col bg-transparent overflow-hidden">
      ...
      <div className="px-6 py-4 border-b border-slate-700/50 bg-slate-800/30 backdrop-blur-sm flex-shrink-0">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-3">
            {/* View Mode Toggle — only visible when KB grid is shown */}
            {!showDocumentsView && (
              <div className="flex items-center bg-slate-900/50 rounded-lg border border-slate-700/50 p-0.5">
                <button
                  onClick={() => setViewMode('tiles')}
                  title="Tiles view"
                  className={`p-1.5 rounded transition-colors ${viewMode === 'tiles' ? 'bg-slate-700 text-white' : 'text-gray-400 hover:text-white'}`}
                >
                ...
                <button
                  ...
                  title="Large view"
                  className={`p-1.5 rounded transition-colors ${viewMode === 'large' ? 'bg-slate-700 text-white' : 'text-gray-400 hover:text-white'}`}
                >
                ...
                <button
                  ...
                  title="Compact view"
                  className={`p-1.5 rounded transition-colors ${viewMode === 'compact' ? 'bg-slate-700 text-white' : 'text-gray-400 hover:text-white'}`}
                >
      ...
      <div className="flex-1 overflow-y-auto px-6 py-6 min-h-0">
        {loading ? (
          ...
          <div className={
            viewMode === 'tiles' ? 'grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4' :
            viewMode === 'large' ? 'grid grid-cols-1 md:grid-cols-2 gap-6' :
            'flex flex-col space-y-2'
          }>
            {knowledgeBases.map((kb) => viewMode === 'list' ? (
              /* List view: horizontal row */
              <div key={kb.kb_id} className="bg-slate-800/70 backdrop-blur-sm rounded-lg border border-slate-700/50 px-4 py-3 hover:border-cyan-500/50 transition-all group flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0 grid grid-cols-12 gap-3 items-center">
                  ...
                  <div className="col-span-2 text-xs text-gray-300">
                    <span className="font-medium">{kb.documents.length}</span> docs
                  </div>
                  <div className="col-span-2">
                    <span className={`text-xs ${kb.documents.length === 0 ? 'text-gray-500' : 'text-green-400'}`}>
                  ...
                  <div className="col-span-2 flex items-center justify-end space-x-1">
                    <button
                      onClick={() => { setSelectedKB(kb.kb_id); setShowUploadModal(true) }}
                    ...
                    <button onClick={(e) => { e.stopPropagation(); startEditKB(kb) }} className="p-1.5 hover:bg-slate-700/50 rounded transition-colors text-gray-400 hover:text-cyan-400 opacity-0 group-hover:opacity-100" title="Rename"><Edit className="w-3.5 h-3.5" /></button>
                    <button onClick={(e) => { e.stopPropagation(); deleteKnowledgeBase(kb.kb_id) }} className="p-1.5 hover:bg-red-500/20 rounded transition-colors text-gray-400 hover:text-red-400 opacity-0 group-hover:opacity-100" title="Delete"><Trash2 className="w-3.5 h-3.5" /></button>
                  </div>
                </div>
              </div>
            ) : viewMode === 'compact' ? (
              /* Compact view: dense single row */
              <div key={kb.kb_id} className="bg-slate-800/70 backdrop-blur-sm rounded border border-slate-700/50 px-3 py-2 hover:border-cyan-500/50 transition-all group flex items-center gap-3">
                <CheckCircle2 className="text-green-500 w-3.5 h-3.5 flex-shrink-0" />
                <span className="text-xs font-medium text-white flex-1 min-w-0 truncate">{kb.name}</span>
                <span className="text-xs text-gray-400 flex-shrink-0">{kb.documents.length} docs</span>
                <span className={`text-xs flex-shrink-0 ${kb.documents.length === 0 ? 'text-gray-500' : 'text-green-400'}`}>
                  {kb.documents.length === 0 ? 'Empty' : 'Active'}
                </span>
                <div className="flex items-center space-x-1 flex-shrink-0">
                  <button onClick={() => { setSelectedKB(kb.kb_id); setShowUploadModal(true) }} className="px-2 py-0.5 text-xs rounded bg-blue-500/20 text-blue-400 hover:bg-blue-500/30 border border-blue-500/30 transition-colors">+ Add</button>
                  <button onClick={(e) => { e.stopPropagation(); startEditKB(kb) }} className="p-1 hover:bg-slate-700/50 rounded transition-colors text-gray-400 hover:text-cyan-400 opacity-0 group-hover:opacity-100" title="Rename"><Edit className="w-3 h-3" /></button>
                  <button onClick={(e) => { e.stopPropagation(); deleteKnowledgeBase(kb.kb_id) }} className="p-1 hover:bg-red-500/20 rounded transition-colors text-gray-400 hover:text-red-400 opacity-0 group-hover:opacity-100" title="Delete"><Trash2 className="w-3 h-3" /></button>
                </div>
              </div>
            ) : (
              /* Tiles / Large: card layout */
              <div
                key={kb.kb_id}
