...
import { MessageSquare, Plus, LogOut, ChevronLeft, ChevronRight, Bot, FileText, Trash2, Star, Users, BookOpen, PlayCircle, Sun, Moon, Bookmark, FlaskConical, Waves, ShieldCheck } from 'lucide-react'
...
export function MainPortal() {
  const [activeView, setActiveView] = useState<'chat' | 'upload' | 'users' | 'onboarding' | 'onboarding-sim' | 'bookmarks' | 'incubation' | 'seagent' | 'qcagent'>('chat')
      ...
      <div className={`${sidebarOpen ? 'w-64' : 'w-0'} bg-slate-800/50 border-r border-slate-700/50 transition-all duration-300 overflow-hidden flex flex-col h-full`}>
        ...
        <div className="px-4 py-2 border-b border-slate-700/50 flex-shrink-0">
          <nav className="space-y-1">
            ...
            <button
              onClick={() => setActiveView('qcagent')}
              className={`w-full flex items-center space-x-3 px-3 py-2 rounded-lg transition-colors text-sm ${
                activeView === 'qcagent'
                  ? 'bg-gradient-to-r from-emerald-500/20 to-cyan-600/20 text-white border border-emerald-400/30'
                  : 'text-gray-400 hover:bg-slate-700/50 hover:text-white'
              }`}
            >
